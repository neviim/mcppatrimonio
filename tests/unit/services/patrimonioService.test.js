...
describe("PatrimonioService", () => {
    ...
    describe("fetchPatrimonio", () => {
        it("deve buscar patrimônio com sucesso", async () => {
            ...
            expect(fetch).toHaveBeenCalledWith("http://example.com/api/v1/patrimonios/patrimonio/12345", expect.objectContaining({
        ...
        it("deve fazer URL encoding do número do patrimônio", async () => {
            ...
            expect(fetch).toHaveBeenCalledWith(expect.stringContaining("ABC%2F123"), expect.any(Object));
        });
    });
    describe("fetchPatrimoniosPorSetor", () => {
        it("deve buscar patrimônios por setor com sucesso", async () => {
            ...
            expect(result).toEqual(mockData);
            expect(fetch).toHaveBeenCalledWith("http://example.com/api/v1/patrimonios/setor/TI", expect.any(Object));
        });
        ...
        it("deve fazer URL encoding do nome do setor", async () => {
            ...
            expect(fetch).toHaveBeenCalledWith(expect.stringContaining("Recursos%20Humanos"), expect.any(Object));
        });
    });
    describe("fetchPatrimoniosPorUsuario", () => {
        it("deve buscar patrimônios por usuário com sucesso", async () => {
            ...
            expect(result).toEqual(mockData);
            expect(fetch).toHaveBeenCalledWith(expect.stringContaining("Jo%C3%A3o%20Silva"), expect.any(Object));
        });
    });
    describe("fetchPatrimonioPorId", () => {
        it("deve buscar patrimônio por ID com sucesso", async () => {
            ...
            expect(result).toEqual(mockData);
            expect(fetch).toHaveBeenCalledWith("http://example.com/api/v1/patrimonios/507f1f77bcf86cd799439011", expect.any(Object));
        });
    });
    describe("updatePatrimonio", () => {
        it("deve atualizar patrimônio com sucesso", async () => {
            ...
            expect(fetch).toHaveBeenCalledWith("http://example.com/api/v1/patrimonios/1", expect.objectContaining({
                method: "PUT",
        ...
        it("deve lançar NotFoundError quando patrimônio não existe", async () => {
            ...
            await expect(service.updatePatrimonio("inexistente", { Usuario: "Teste" })).rejects.toThrow(NotFoundError);
        });
    });
    describe("createPatrimonio", () => {
        it("deve criar patrimônio com sucesso", async () => {
            ...
            expect(result).toEqual(mockResponse);
            expect(fetch).toHaveBeenCalledWith("http://example.com/api/v1/patrimonios/", expect.objectContaining({
                method: "POST",
        ...
        it("deve lançar ExternalError em caso de erro 400", async () => {
            ...
            await expect(service.createPatrimonio({ Patrimonio: "" })).rejects.toThrow(ExternalError);
        });
    });
    ...
    describe("configuração", () => {
        it("deve remover trailing slash da baseUrl", () => {
            ...
            expect(fetch).toHaveBeenCalledWith("http://example.com/api/v1/patrimonios/patrimonio/12345", expect.any(Object));
        });
...
//# sourceMappingURL=patrimonioService.test.js.map