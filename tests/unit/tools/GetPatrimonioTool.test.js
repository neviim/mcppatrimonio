...
describe("GetPatrimonioTool", () => {
    ...
    describe("execute", () => {
        ...
        it("deve retornar erro quando patrimônio não existe", async () => {
            const mockFetchPatrimonio = vi.fn().mockRejectedValue(new NotFoundError("Patrimônio não encontrado"));
            vi.mocked(PatrimonioService).mockImplementation(() => ({
                fetchPatrimonio: mockFetchPatrimonio,
            }));
        ...
        it("deve retornar erro de validação para parâmetros inválidos", async () => {
            const result = await tool.execute({ numero: "" }, mockContext);
            expect(result.isError).toBe(true);
        ...
        it("deve criar PatrimonioService com credenciais corretas", async () => {
            ...
            vi.mocked(PatrimonioService).mockImplementation(mockConstructor);
            await tool.execute({ numero: "12345" }, mockContext);
            expect(mockConstructor).toHaveBeenCalledWith("http://example.com", "test-token");
        });
    });
...
//# sourceMappingURL=GetPatrimonioTool.test.js.map